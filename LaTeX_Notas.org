...
**** Disección
     \newcommand es quien hace la magia. El primer argumento es el nombre que le
     asignaremos al comando, el segundo argumento es el texto que pondremos en
     el documento.

     Ahora cuando escribamos \TUG en nuestro documento, el nombre completo debera
     de mostrarse. Si en el futuro decidiéramos cambiar el nombre o su formato, solo
     tendríamos que modificar la linea de l definición del comando y el comando
     sera valido en cualquier parte del documento.

     Tal vez queramos usar comandos para formatear el texto cuando se defina el comando.
     Digamos que queremos cambiar el formato del texto a versalita en todas sus ocurrencias.
     Solo tendremos que modificar la definición:

     =\newcommand{\TUG}{\textsc{TeX User Group}}=

     También se uso el comando \TeX, el cual solo imprime el nombre
     del sistema básico de composición como en su logo. Esta es una abreviación
     del comando como la que hemos escrito así como también la es \LaTeX.
     Note que se ha colocado una diagonal inversa después de \TeX.
     El siguiente espacio separa el comando del texto siguiente y no sera mostrado
     en la salida. Usando la diagonal inversa seguida de un espacio en blanco, forzara
     al espacio a aparecer en la salida de otra manera sera ignorado.



*** Espacio despues de los comandos
    Dejar un espacio después de un comando mediante la diagonal inversa puede ser
    fácil olvidado. ¿Podríamos modificar el comando para automatizarlo?
    Tareas que no son soportadas directamente por LaTeX, puedes ser resueltas
    mediante el uso de paquetes(packages). Los cuales son una colección de estilos
    y comandos.
